...
async function loadCartHistory() {
  const listEl = document.getElementById("co2-cart-history-list");
  const statusEl = document.getElementById("co2-cart-history-status");

  if (!listEl) return;

  try {
    statusEl.textContent = "Chargement de votre historique…";

    const resp = await honouaFetch(
      `${API_BASE_URL}/api/cart/history?limit=10`
    );
    const items = await resp.json();
    window.__HONOUA_CART_COUNT = Array.isArray(items) ? items.length : 0;
    ...
    items.forEach(item => {
      ...
      div.className = "co2-cart-history-item";
      div.innerHTML = `
        <div class="row">
        ...
        <div class="row">
          ...
          <span class="value">${(item.total_co2_g / 1000).toFixed(2)} kg</span>
        </div>
        <div class="row">
          <span class="label">Produits</span>
          <span class="value">${item.nb_articles}</span>
        </div>
      `;
    ...
    console.warn("[Suivi CO₂] Historique indisponible", e);
    listEl.innerHTML = `
      <p class="co2-cart-history-empty">
...
document.addEventListener("DOMContentLoaded", () => {
  const btnActivate = document.getElementById("co2-challenges-activate-default");
  const btnRefresh = document.getElementById("co2-challenges-refresh");

  if (btnActivate) {
    btnActivate.addEventListener("click", async () => {
      try {
        await honouaFetch(`${API_BASE_URL}/challenges/personal/activate`, {
          method: "POST",
          body: JSON.stringify({ challenge_id: 1 })
        }).then(r => r.json());

        await loadPersonalChallenges(true);
      } catch (e) {
        console.warn("[Suivi CO2] activation défi échouée", e);
        alert("Impossible d’activer le défi pour le moment.");
      }
    });
  }
  ...
  loadPersonalChallenges(false);
});


async function loadPersonalChallenges(forceEvaluate) {
  const listEl = document.getElementById("co2-challenges-list");
  if (!listEl) return;

  listEl.dataset.state = "loading";
  listEl.innerHTML = `<div class="co2-challenge-empty">Chargement…</div>`;

  try {
    // 1) Récupérer actifs
    const active = await honouaFetch(`${API_BASE_URL}/challenges/personal/active`)
      .then(r => r.json());
    ...
    if (!active || active.length === 0) {
      ...
      listEl.innerHTML = `
        <div class="co2-challenge-empty">
          ...
          <br/>Exemple : “Réduire -10% CO2 sur 30 jours”.
        </div>`;
      return;
    }

    // 2) Évaluer si demandé (ou si jamais évalué)
    const evaluated = [];
    for (const inst of active) {
      const shouldEval = forceEvaluate || !inst.last_evaluated_at;
      if (shouldEval) {
        const ev = await honouaFetch(`${API_BASE_URL}/challenges/personal/${inst.instance_id}/evaluate`, {
          method: "POST"
        }).then(r => r.json());
        evaluated.push({ ...inst, evaluation: ev });
      } else {
        evaluated.push({ ...inst, evaluation: null });
      }
    }
    ...
    evaluated.forEach(item => {
      ...
      const status = (ev?.status || item.status || "").toUpperCase();
      const cartCount = Number(window.__HONOUA_CART_COUNT || 0);
      const rawMsg = ev?.message || "";

      // UX MVP : si aucun panier, on force un message d’action
      const msg = (cartCount === 0)
        ? "🧺 Ajoutez au moins un panier pour démarrer ce défi. Les défis se basent sur vos achats réels."
        : rawMsg;

      const pct = (ev?.progress_percent != null) ? Number(ev.progress_percent) : (item.progress_percent != null ? Number(item.progress_percent) : null);

      const pctSafe = (pct == null || isNaN(pct)) ? 0 : Math.max(0, Math.min(100, pct));
      const pctLabel = (pct == null || isNaN(pct)) ? "—" : `${pctSafe.toFixed(0)}%`;

      const card = document.createElement("div");
      card.className = "co2-challenge-card";
      card.innerHTML = `
        <div class="co2-challenge-head">
          <div class="co2-challenge-name">${item.name || item.code}</div>
          <div class="co2-challenge-status">${status}</div>
        </div>

        <div class="co2-challenge-bar">
          <div class="co2-challenge-bar-fill" style="width:${pctSafe}%"></div>
        </div>
        <div class="co2-challenge-bar-label">Progression : ${pctLabel}</div>

        ${
          msg
            ? `<div class="co2-challenge-message">${msg}</div>`
            : ""
        }

        ${
          (Number(window.__HONOUA_CART_COUNT || 0) === 0)
            ? `
              <div class="co2-challenges-actions" style="margin-top:8px">
                <button type="button" class="co2-challenges-refresh-btn" onclick="location.href='scan-impact.html'">
                  Ajouter un panier
                </button>
              </div>
            `
    ...
    console.warn("[Suivi CO2] loadPersonalChallenges error", e);
