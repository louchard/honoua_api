...
(async () => {
  ...
  try { if ($video) $video.style.display = visible ? '' : 'none'; } catch(_) {}
}
    ...
    return await new Promise((resolve) => {
      Q.init({
        ...
        if (err) {
          ...
          // IMPORTANT : Quagga a masqué la vidéo native → on la ré-affiche avant le fallback ZXing
          stopQuagga();
  ...
  let lastChallengeAutoEval = 0;
    ...
    // Objectif : éviter tout crash + activer/désactiver le bouton "Lampe" selon support réel
    ...
    try {
      ...
      // Sur certains navigateurs, caps.torch existe (booléen) si supporté
      ...
      // En cas de comportement étrange iOS, on sécurise : pas de torche
      torchSupported = false;
      try { $torch.disabled = true; } catch(_) {}
      return false;
    }
  }
...
async function startWith(deviceId){
  try{
    await stopStream();
     ...
     // iPhone : tenter Quagga2 en priorité, puis fallback vers caméra native + ZXing
